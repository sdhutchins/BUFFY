...
from buffy.media_entry import create_media
from buffy.media.base import MediaBase, disk_write
from buffy.util.config import Config, DEFAULT_COMPRESS, DEFAULT_ENCODING
...
class BUFFY(object):
    def __init__(self):
        ...
        # simple flow options
        args.add_argument("-src", dest="src", default=None,
                          help="backup source")
        args.add_argument("-dst", dest="dst", action='append', default=None,
                          help="backup destination")
        args.add_argument("-n", "--name", dest="name", default=None,
                          help="backup name")
        args.add_argument("-e", "--encoding", dest="encoding",
                          action="store_const", const=True,
                          help="name encoding with date (default: %s)" % DEFAULT_ENCODING)
        args.add_argument("-cmp", "--compress", dest="compress",
                          action="store_const", const=True,
                          help="compress backup files (default: %s)" % DEFAULT_COMPRESS)
        args.add_argument("-r", "--report", dest="rpt", default=None,
                          help="report path")
        simple_flow_args = ["src", "dst", "name", "compress", "encoding",
                            "rpt"]
        # general options
        ...
        args.add_argument("-s", "--silent", dest="silent", action="store_const",
                          const=WARNING, help="silent mode")
        args.add_argument("-d", "--dry_run", dest="dry", action="store_const",
                          const=True, help="perform a dry run")
        # normal flow option
